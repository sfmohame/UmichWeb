...
def get_terms():
    ...
    '''
    url = '/Curriculum/SOC/v1/Terms'
    data = get_data(url)['getSOCTermsResponse']['Term']
...
def get_schools(termcode):
    return get_data('/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Schools')['getSOCSchoolsResponse']['School'] 

def get_subjects(termcode, sub):
    return get_data('/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Schools/'+str(sub)+'/Subjects')['getSOCSubjectsResponse']['Subject']

def get_classes(termcode, sub, subSelected):
    return get_data('/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Schools/'+str(sub)+'/Subjects/'+str(subSelected)+'/CatalogNbrs')['getSOCCtlgNbrsResponse']['ClassOffered']

def get_courseD(termcode, sub, subSelected, catNum):
    return get_data('/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Schools/'+str(sub)+'/Subjects/'+str(subSelected)+'/CatalogNbrs/'+str(catNum)+'')['getSOCCourseDescrResponse']['CourseDescr']

def get_sections(termcode, sub, subSelected, catNum):
    url = '/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Schools/'+str(sub)+'/Subjects/'+str(subSelected)+'/CatalogNbrs/'+str(catNum)+'/Sections'
    data = get_data(url)['getSOCSectionsResponse']['Section']
    if type(data) is not list:
        temp_list = [ data ]
        data = temp_list
    return data

def get_classNums(termcode, subSelected, catNum):
    return get_data('/Curriculum/SOC/v1/Terms/'+str(termcode)+'/Classes/Search/'+str(subSelected)+'%20'+str(catNum)+'')['searchSOCClassesResponse']['SearchResult']
